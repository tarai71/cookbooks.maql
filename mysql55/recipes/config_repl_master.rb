...
if node_type == "master" then
  template "#{node['mysql55_server_conf_dir']}/setting-repl-master.sql" do
  ...
  bash "setting-replication \'master\'" do
    ...
    code <<-EOH
    mysql -u root -p\"#{node['mysql55_server_root_password']}\" < ./setting-repl-master.sql
    EOH
    action :nothing
  end
  ...
  bash "take-snapshot(mysqldump) \'master\'" do
    user "root"
    cwd node['mysql55_server_conf_dir']
#    creates "#{node['mysql55_server_conf_dir']}/setting-repl-master.dump"
    code <<-EOH
    mysqldump -u root -p\"#{node['mysql55_server_root_password']}\" \
    --single-transaction --flush-logs --all-databases --master-data=2 > ./setting-repl-master.dump
    EOH
		not_if "test -f #{node['mysql55_server_conf_dir']}/setting-repl-master.dump"
  ...
  ruby_block "check-snapshot(mysqldump) \'master\'" do
    block do
			puts "\n---------------------------------"
			puts "## dump file info"
			puts `LANG=C; ls -lh #{node['mysql55_server_conf_dir']}/setting-repl-master.dump` 
			puts "\n---------------------------------"
		end
		action :nothing
  end
  ...
  (slave_ip | bkup_ip).each do |s|
   scp "#{node['mysql55_server_conf_dir']}/setting-repl-master.dump" do
     dest_host s
   end
  end
