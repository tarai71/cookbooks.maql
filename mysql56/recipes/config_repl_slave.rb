...
## slave 
  bash "restore-snapshot(mysqldump) \'slave, bkup\'" do
    ...
    mysql -u root -p\"#{node['mysql56_server_root_password']}\" < ./setting-repl-master.dump
    EOH
  end

  template "#{node['mysql56_server_conf_dir']}/setting-repl-slave.sql" do
  ...
  bash "setting-replication \'slave\'" do
    ...
    cwd node['mysql56_server_conf_dir']
    code <<-EOH
    mysql -u root -p\"#{node['mysql56_server_root_password']}\" -e \"stop slave;\"
    mysql -u root -p\"#{node['mysql56_server_root_password']}\" < ./setting-repl-slave.sql
    EOH
    action :nothing
  end
...
## slave
  bash "start-slave" do
    ...
    cwd node['mysql56_server_conf_dir']
    code <<-EOH
    mysql -u root -p\"#{node['mysql56_server_root_password']}\" -e "start slave;"
    sleep 3
    EOH
    action :run
    only_if "mysql -u root -p\"#{node['mysql56_server_root_password']}\" -e \"show #{node_type} status\\G\" |grep Running |grep No"
  end
  ...
  ruby_block "check-replication \'#{node_type}\'" do
    block do 
      ...
      puts `mysql -u root -p\"#{node['mysql56_server_root_password']}\" -e \"show #{node_type} status\\G\"` 
      puts "\n-------------------------------------------"
    end
  end
